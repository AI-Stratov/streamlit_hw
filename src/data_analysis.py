...
def analyze_city(city_data):
    ...
    # min_periods=1 убирает NaN в начале ряда (иначе первые 29 точек - пробел на графике)
    ...
    for season in city_data['season'].unique():
        ...
        city_data.loc[mask, 'is_anomaly'] = (
                (city_data.loc[mask, 'temperature'] > upper) |
...
def run_multiprocessing(cities):
    ...
    # spawn выбран намеренно для кроссплатформенности (macOS/Windows не поддерживают fork).
    # На Linux дефолтный fork был бы быстрее - это одна из причин, почему multiprocessing
    # проигрывает sequential в бенчмарке на Streamlit Cloud.
...
def run_joblib(cities):
    workers = min(4, len(cities))
    result = Parallel(n_jobs=workers)(
        delayed(analyze_city)(city) for city in cities
    )
    return pd.concat(result)
